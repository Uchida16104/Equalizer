...
export default function P5Visualizer({
  ...
  const initP5 = useCallback(async () => {
    ...
    const sketch = (p: InstanceType<typeof p5>) => {
      ...
      function getGradient(ctx: CanvasRenderingContext2D, w: number, h: number): CanvasGradient {
      ...
      p.draw = function () {
        ...
        drawPeaks(p, currentPeaks, spectrum, w, specH, sampleRate);
        drawWaveform(p, spectrum, w, specH + 8, waveH);
        drawFrequencyLabels(p, w, specH);
      };

      function drawIdleScreen(p: InstanceType<typeof p5>) {
        p.fill(30, 30, 50);
        p.noStroke();
        p.textSize(14);
      ...
      function drawGrid(p: InstanceType<typeof p5>, w: number, h: number, sr: number) {
      ...
      function drawSpectrum(
        ...
        const gradient = getGradient(canvas, w, h);
        canvas.save();
        canvas.beginPath();
        canvas.moveTo(0, h);
